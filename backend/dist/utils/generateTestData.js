...
function generatePrice(min = 10, max = 500) {
    return Math.round((Math.random() * (max - min) + min) * 100) / 100;
}
function generateInventory() {
    return Math.floor(Math.random() * 1000) + 10;
}
function generateSalesData(price, inventory) {
...
async function generateTestData() {
    try {
        ...
        await dataService_1.dataService.writeData('products', []);
        ...
        for (const store of stores) {
            ...
            const skuPrefix = skuPrefixes[store.name] || 'GEN';
        ...
        await dataService_1.dataService.writeData('products', allProducts);
