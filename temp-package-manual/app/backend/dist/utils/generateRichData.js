...
exports.generateAllRichData = generateAllRichData;
const fs_extra_1 = __importDefault(require("fs-extra"));
const path_1 = __importDefault(require("path"));
function generateLargeNumber(min = 10000, max = 100000) {
...
function generateSellerForums(storeId) {
    const forumTopics = [
...
function generateSellerNews(storeId) {
    const newsItems = [
...
function generateDashboardData(storeId) {
...
function generateSalesData(storeId) {
    const multiplier = getStoreMultiplier(storeId);
    const salesData = [];
    for (let i = 0; i < 30; i++) {
        const date = new Date();
...
function generateProductData(storeId) {
    const productNames = [
...
async function generateAllRichData() {
    const stores = ['store-us-main', 'store-jp-main', 'store-uk-main', 'store-de-main'];
    ...
    await fs_extra_1.default.writeJSON(path_1.default.join(dataDir, 'dashboard_snapshots.json'), dashboardData, { spaces: 2 });
    const salesData = {};
    stores.forEach(storeId => {
        salesData[storeId] = generateSalesData(storeId);
    });
    await fs_extra_1.default.writeJSON(path_1.default.join(dataDir, 'sales_data.json'), salesData, { spaces: 2 });
    const allProducts = [];
    stores.forEach(storeId => {
        const storeProducts = generateProductData(storeId);
        allProducts.push(...storeProducts);
    });
    await fs_extra_1.default.writeJSON(path_1.default.join(dataDir, 'products.json'), allProducts, { spaces: 2 });
    const communicationsData = generateCommunicationsData();
    await fs_extra_1.default.writeJSON(path_1.default.join(dataDir, 'communications.json'), communicationsData, { spaces: 2 });
    ...
    return {
        ...
        sales_entries: Object.values(salesData).reduce((acc, data) => acc + data.length, 0),
